...
data = \
np.genfromtxt('B=4/B_is_4_E_omega_is_0.1_alpha_0.9496_E_dc_is_8.2_vary_mu_and_omega.data',
              delimiter=' ', comments='#',
              names=['E_dc', 'E_omega', 'omega', 'mu', 'v_dr_inst', 'A', 'NORM', 'v_y_inst', 
                     'm_eff_inst', 'v_dr', 'v_y', 'm_eff', 'ASIN'])

data2 = \
np.genfromtxt('B=4/B_is_4_E_omega_is_0.1_mu_is_116_alpha_0.9496_E_dc_is_8.2_vary_omega.data',
              delimiter=' ', comments='#',
              names=['E_dc', 'E_omega', 'omega', 'mu', 'v_dr_inst', 'A', 'NORM', 'v_y_inst', 
                     'm_eff_inst', 'v_dr', 'v_y', 'm_eff', 'ASIN'])
...
index = np.asarray([row['mu'] > 32 and row['mu'] < 33 for row in data])
fdata = data[index]
p1 = ax.plot(fdata['omega'], fdata['A'], color='#00bbff', lw=3)

p1 = ax.plot(data2['omega'], data2['A'], color='#0000ff', lw=3)
...
p1 = ax.plot(data2['omega'], data2['ASIN'], color='#0000ff', lw=3)
...
ax.set_xlim(0,10)
ax.set_ylim(-0.036,0.007)
ax.set_xlabel('$\\tilde{\omega}$', fontsize='xx-large')

#plt.show()
outfile="plots/A_and_ASIN_of_omega_E_dc_is_8_point_2_vary_mu.pdf"
