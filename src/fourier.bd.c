...
void L_f(ffloat host_E_dc, ffloat host_E_omega, ffloat host_omega, ffloat B, ffloat dt, float t, 
         ffloat *a0, int MSIZE, ffloat *a_current, ffloat *b_current, ffloat *a_next, ffloat *b_next, 
         ffloat abdt, ffloat nu);

void L_b(ffloat host_E_dc, ffloat host_E_omega, ffloat host_omega, ffloat B, ffloat dt, float t, 
         ffloat *a0, int MSIZE, ffloat *a_current, ffloat *b_current, ffloat *a_next, ffloat *b_next, 
         ffloat abdt, ffloat nu);
...
int main(int argc, char *argv[]) {
  ...
  // follwing two vectors are used to store a and b after L_{b}\circL_{a}
  ffloat *a_s; a_s = calloc(SIZE_2D, sizeof(ffloat));
  ffloat *b_s; b_s = calloc(SIZE_2D, sizeof(ffloat));
         ...
         ffloat abdt, ffloat nu)
{
  for( int m = 1; m < 2*M+2; m++ ) {
    for( int n = 0; n < N; n++ ) {
      ...
      ffloat mu_t_plus_1   = n*beta_t_plus_1*dt/2;
      ffloat mu_t          = n*beta_t*dt/2;
      ffloat g = dt*nm(a0,n,m) + nm(a_current,n,m)*(1-dt/2) - nm(b_current,n,m)*mu_t 
        + abdt*(nm(b_current,n+1,m+1) - nm(b_current,n+1,m-1) 
                - ( n < 2 ? 0 : ( nm(b_current,n-1,m+1) - nm(b_current,n-1,m-1))));

      ffloat h = nm(b_current,n,m)*(1-dt/2) + nm(a_current,n,m)*mu_t 
        + abdt*((n==1?2:1)*(n==0?0:(nm(a_current,n-1,m+1)-nm(a_current,n-1,m-1)))
                - (nm(a_current,n+1,m+1)-nm(a_current,n+1,m-1)));

      nm(a_next,n,m) = (g*nu-h*mu_t_plus_1)/(nu*nu+mu_t_plus_1*mu_t_plus_1);
      if( n > 0 ) {
        nm(b_next,n,m) = (g*mu_t_plus_1+h*nu)/(nu*nu+mu_t_plus_1*mu_t_plus_1);
      }
    }
  }
} // end of L_f
         ...
         ffloat abdt, ffloat nu) 
{
  ...
  for( int m = 1; m < 2*M+2; m++ ) {
    for( int n = 0; n < N; n++ ) {
      ...
      ffloat sigma = nu*nu+mu_t_plus_1*mu_t_plus_1;
      
      ffloat F=nm(a_next,n,m)*sigma-nm(a0,n,m)*dt*nu-A*nu+B*mu_t_plus_1;
      ffloat G=nm(b_next,n,m)*sigma-nm(a0,n,m)*dt*mu_t_plus_1-A*mu_t_plus_1+B*nu;

      ffloat rho=(xi*xi+mu_t*mu_t)*(nu*nu-mu_t_plus_1*mu_t_plus_1);

      nm(a_current,n,m) = (F*(xi*nu+mu_t*mu_t_plus_1)-G*(xi*mu_t_plus_1+mu_t*nu))/rho;
      nm(b_current,n,m) = (F*(xi*mu_t_plus_1-mu_t*nu)-G*(xi*nu+mu_t*mu_t_plus_1))/rho;
    }
  }
}
