...
__device__ ffloat E_dc, E_omega, omega, B, dt, dPhi, nu, nu2, nu_tilde, bdt, mu, alpha;
...
__global__ void solve(ffloat *a0, ffloat *a_current,    ffloat *b_current, 
                      ...
                      ffloat t, ffloat t_hs, ffloat cos_omega_t, ffloat cos_omega_t_plus_dt, int action);
...
int main(int argc, char *argv[]) {
  ...
  HANDLE_ERROR(cudaMemcpy(a[current], host_a0, SIZE_2Df, cudaMemcpyHostToDevice));
  ...
  memcpy(host_a_hs, host_a, SIZE_2D*sizeof(ffloat)); // temporary solution // FIX ME!!!
  HANDLE_ERROR(cudaMemcpy(a[current_hs], host_a_hs, SIZE_2Df, cudaMemcpyHostToDevice)); 
  HANDLE_ERROR(cudaMemcpy(b[current_hs], host_b_hs, SIZE_2Df, cudaMemcpyHostToDevice));
  
  ffloat host_nu = 1+host_dt/2;
  HANDLE_ERROR(cudaMemcpyToSymbol(nu,       &host_nu,       sizeof(ffloat)));

  ffloat host_nu2 = host_nu * host_nu;
  ...
  for( ffloat t = 0; t < t_max; t += host_dt ) {
    //for( ffloat t = 0; t < 10*host_dt; t += host_dt ) {
    t_hs = t + host_dt/2;
    //printf("%0.20f current=%d next=%d current_hs=%d next_hs=%d\n", t, current, next, current_hs, next_hs);
    ffloat cos_omega_t = cos(host_omega*t);
    ffloat cos_omega_t_plus_dt = cos(host_omega*(t+host_dt));
    solve<<<blocks,TH_PER_BLOCK>>>(a0, a[current], b[current], a[next], b[next], a[current_hs], b[current_hs], a[next_hs], b[next_hs], t, t_hs, 
                                   cos_omega_t, cos_omega_t_plus_dt, 1);
    cos_omega_t = cos(host_omega*t_hs);
    cos_omega_t_plus_dt = cos(host_omega*(t_hs+host_dt));
    solve<<<blocks,TH_PER_BLOCK>>>(a0, a[current], b[current], a[next], b[next], a[current_hs], b[current_hs], a[next_hs], b[next_hs], t, t_hs, 
                                   cos_omega_t, cos_omega_t_plus_dt, 2);
                      ...
                      int action)
{
  ...
  if( action == 1 ) { // step from (t,t+1/2) to (t+1)
    ffloat mu_t_part = (E_dc + E_omega*cos_omega_t+B*dev_phi_y(m))*dt/2; // Should be taken at time t+dt/2 ?
  ...
  } else if( action == 3 ) { // step from (t+1/2,t+1) to (t+3/2)
    for( int n = 0; n < N; n++ ) {
      dnm(a_next_hs,n,m) = dnm(a_next,n,m);
      dnm(a_current_hs,n,m) = dnm(a_next,n,m);
      if( n > 0 ) {
        dnm(b_next_hs,n,m) = dnm(b_next,n,m);
        dnm(b_current_hs,n,m) = dnm(b_next,n,m);
      }
    }

  } else if( action == 2 ) { // step from (t+1/2,t+1) to (t+3/2)
    //ffloat mu_t_part = (E_dc + E_omega*cos(omega*t_hs)+B*dev_phi_y(m))*dt/2;
    ffloat mu_t_part = (E_dc + E_omega*cos_omega_t+B*dev_phi_y(m))*dt/2;
    //ffloat mu_t_plus_1_part = (E_dc + E_omega*cos(omega*(t_hs+dt))+B*dev_phi_y(m))*dt/2;
    ffloat mu_t_plus_1_part = (E_dc + E_omega*cos_omega_t_plus_dt+B*dev_phi_y(m))*dt/2;
    for( int n = 0; n < N; n++ ) {
      ffloat mu_t = n*mu_t_part;
      ffloat mu_t_plus_1 = n*mu_t_plus_1_part;
