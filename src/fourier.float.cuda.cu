  ...
  if( display == 0 ) {
    ...
    for( ffloat phi_x = -PI; phi_x < PI; phi_x += 0.05 ) {
      for( int m = 1; m < 2*host_M+2; m++ ) {
        ...
        ffloat value0 = 0;
        for( int n = 0; n < host_N+1; n++ ) {
          value0 += nm(host_a0,n,m)*cosf(n*phi_x);
        }
        fprintf(out, "%0.10f %0.10f %0.18f\n", phi_x, phi_y(m), value<0?0:value, value0<0?0:value0);
      }
    }
    fclose(out);
    return 0;
...
//__global__ void solve1(ffloat *f_current, ffloat *f_next, ffloat *f_0, ffloat *v_dr_int, ffloat *sin_p3, ffloat A, ffloat B, ffloat C, ffloat t) {
__global__ void solve1(int TMSIZE, ffloat *a0, ffloat *a_current, ffloat *b_current, ffloat *a_next, ffloat *b_next, 
  ...
  //ffloat *v_dr_int, ffloat *sin_p3, ffloat A, ffloat B, ffloat C, ffloat t) {
  const int m = threadIdx.x+blockDim.x*blockIdx.x;
  if( m==0 || m > TMSIZE ) { return; }  // TMSIZE should be 2*M+1

  ffloat beta_t_plus_1 = E_dc + E_omega*cosf(omega*(t+dt))+B*dev_phi_y(m);
  ffloat beta_t        = E_dc + E_omega*cosf(omega*(t))+B*dev_phi_y(m);
