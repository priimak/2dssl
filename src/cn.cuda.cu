...
ffloat host_dt =   0.000025; //0.0001;
...
int main(int argc, char *argv[]) {
  ...
  ffloat *av_data;
  HANDLE_ERROR(cudaMalloc((void **)&av_data, 10));
  ...
  for( ffloat t = 0; t < t_max; t += host_dt ) {
    ...
    if( host_E_omega != null && display != 7 ) {
  ...
  if( display == 4 ) {
    ...
    v_dr_inst       *= 2*gsl_sf_bessel_I0(host_mu)*PI*sqrt(host_alpha)/gsl_sf_bessel_In(1, host_mu);
    v_y_inst        *= 4*PI*gsl_sf_bessel_I0(host_mu)/gsl_sf_bessel_In(1, host_mu);
    m_over_m_x_inst *= PI*host_alpha*sqrt(host_alpha);
    
    fprintf(out, "#E_{dc}                \\tilde{E}_{\\omega}     \\tilde{\\omega}         mu                     <v_{dr}/v_{p}>         A(\\omega)              NORM    <v_{y}/v_{p}>    <m/m_{x,k}\n");
    fprintf(out, "%0.20f %0.20f %0.20f %0.20f %0.20f %0.20f %0.20f %0.20f %0.20f\n", host_E_dc, host_E_omega, host_omega, host_mu, v_dr_inst, 0.0, 
            norm, v_y_inst, m_over_m_x_inst);
    //if( out != stdout ) {
    //  fclose(out);
    //}
    return 0;
...
__global__ void av(ffloat *a, ffloat *b, ffloat *av_data, ffloat t, ffloat dt) {
  
  ffloat v_dr_av = 0; ffloat v_y_av = 0; ffloat m_over_m_x_av = 0; 
  int av_count = av_data[0] + 1; 

  ffloat v_dr_inst = 0; ffloat v_y_inst = 0; ffloat m_over_m_x_inst = 0; ffloat A = 0;
  for( int m = 1; m < 2*M+2; m++ ) {
    v_dr_inst += nm(host_b,1,m)*dPhi;
    v_y_inst  += nm(host_a,0,m)*phi_y(m)*dPhi;
    m_over_m_x_inst += nm(host_a,1,m)*dPhi;
  }
